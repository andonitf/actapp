...
export class ActDataservice {
  ...
  SERVER_URL : string = "http://www.ligasanmiguel.com/json/";

  hizkuntzak: Array<{value: string, label: string}>;
  ligak: Array<{value: string, label: string}>;
  denboraldiak: number[] = [];

	denboraldia: number = null;
	liga: string = null;
	hizkuntza: string = null;
  ...
  constructor(
    ...
    console.log('Hello ActDataservice Provider');
    ...
    this.ligak = [
      {
        value: 'LSM',
        label: 'Liga San Miguel'
      },
      {
        value: 'LET',
        label: 'Liga Eusko Tren',
      }
    ];
    for (let i=new Date().getFullYear(); i>=2010; i--){
        this.denboraldiak.push(i);
    }
	...
	Observable.forkJoin([
      ...
      .subscribe(data => {
        ...
        this.denboraldia = parseInt( data[0].toString(), 10);
        this.liga = data[1].toString();
        this.hizkuntza = data[2].toString();
        //both call succeeded
        //console.log(`forkJoin ${data[0]}, ${data[1]}, ${data[2]} `)
        ...
        this.theme = new BehaviorSubject(this.liga +'-theme');        
      }
    );
  }

  getEstropadakZerrenda() {
    console.log('URL: '+ this.SERVER_URL+'estropadak'+this.liga+'.php?d='+this.denboraldia);
    return this.http.get(this.SERVER_URL+'estropadak'+this.liga+'.php?d='+this.denboraldia)
      .timeout(15000)
      .map(res => res.json())
  ...
  getEstropadaEmaitzak(idEstropada: string) {
    console.log('URL: '+ this.SERVER_URL+'emaitzak'+this.liga+'.php?e='+idEstropada);
    return this.http.get(this.SERVER_URL+'emaitzak'+this.liga+'.php?e='+idEstropada)
      .map(res => res.json())
      //.catch(this.handleError);
      .catch(error => {
          console.error(error);
  ...
  getTripulazioa(idEstropada: string, idTaldea: string) {
    console.log('URL: '+ this.SERVER_URL+'tripulazioa'+this.liga+'.php?e='+idEstropada+'&t='+idTaldea+'&h='+this.hizkuntza);
    return this.http.get(this.SERVER_URL+'tripulazioa'+this.liga+'.php?e='+idEstropada+'&t='+idTaldea+'&h='+this.hizkuntza)
      .timeout(15000)
      .map(res => res.json())
      //.catch(this.handleError);
      .catch(error => {
  ...
  getTaldeakZerrenda() {
    console.log('URL: '+ this.SERVER_URL+'taldeak'+this.liga+'.php?d='+this.denboraldia);
    return this.http.get(this.SERVER_URL+'taldeak'+this.liga+'.php?d='+this.denboraldia)
      .timeout(15000)
      .map(res => res.json())
      //.catch(this.handleError);
      .catch(error => {
  ...
  getArraunlariakZerrenda(idTaldea: string) {
    console.log('URL: '+ this.SERVER_URL+'arraunlariak'+this.liga+'.php?d='+this.denboraldia+'&t='+idTaldea+'&h='+this.hizkuntza);
    return this.http.get(this.SERVER_URL+'arraunlariak'+this.liga+'.php?d='+this.denboraldia+'&t='+idTaldea+'&h='+this.hizkuntza)
      .timeout(15000)
      .map(res => res.json())
      .catch(error => {
          console.error(error);
  ...
  getArgazkiak(idEstropada: string) {
    console.log('URL: '+ this.SERVER_URL+'argazkiak'+this.liga+'.php?e='+idEstropada);
    return this.http.get(this.SERVER_URL+'argazkiak'+this.liga+'.php?e='+idEstropada)
      .timeout(15000)
      .map(res => res.json())
      .catch(error => {
          console.error(error);
  ...
  getSailkapena() {
    console.log('URL: '+ this.SERVER_URL+'sailkapena'+this.liga+'.php?d='+this.denboraldia);
    return this.http.get(this.SERVER_URL+'sailkapena'+this.liga+'.php?d='+this.denboraldia)
      .timeout(15000)
      .map(res => res.json())
      .catch(error => {
          console.error(error);
  ...
  getBerriakZerrenda() {
    console.log('URL: '+ this.SERVER_URL+'berriakLSM.php?d='+this.denboraldia+'&h='+this.hizkuntza);
    return this.http.get(this.SERVER_URL+'berriakLSM.php?d='+this.denboraldia+'&h='+this.hizkuntza)
      .timeout(15000)
      .map(res => res.json())
      .catch(error => {
          console.error(error);
  ...
  getZuzenekoDatuak() {
    //console.log('http://andonitorres.org/2015/ligasanmiguelapp/JsonDatuak/G93/AppMovil1.txt');
    console.log('../assets/G93/AppMovil1.txt');
    return this.http.get('../assets/G93/AppMovil1.txt')
    //return this.http.get('../assets/G93/AppMovil1.txt')
      .timeout(15000)
      //.map(res => res.json())
      .catch(error => {
  ...
  getDenboraldia() : Promise<number>{
    return this.storage.get('Denboraldia').then( (data:number) => {
        return data;
    });
  }

  setDenboraldia(denboraldia_data: number) : Promise<number>{
	this.denboraldia = denboraldia_data;
    return this.storage.set('Denboraldia', denboraldia_data).then( (data:number) => {
        return data;
    });
  }

  getLiga() : Promise<string>{
    return this.storage.get('Liga').then( (data:string) => {
        return data;
    });
  }

  setLiga(liga_data: string) : Promise<string>{
	  this.liga = liga_data;
    return this.storage.set('Liga', liga_data).then( (data:string) => {
      this.setTheme(data);
		  return data;
    });
  }

  getHizkuntza() : Promise<string>{
    return this.storage.get('Hizkuntza').then(  (data:string) => {
  		return data;
    });
  }
  
  setHizkuntza(hizkuntza_data: string) : Promise<string>{
	this.hizkuntza = hizkuntza_data;
    return this.storage.set('Hizkuntza', hizkuntza_data).then(  (data:string) => {
        return data;
    });
  }

  getHizkuntzaZerrenda(){
      return this.hizkuntzak;
  }
  getLigaZerrenda(){
      return this.ligak;
  }
  getDenboraldiZerrenda(){
      return this.denboraldiak;
  }
  ...
  entityToHtml(string) {
    for (var i in this.entity_table) {
        if (i != '38') {
            string = string.replace(new RegExp(this.entity_table[i], "g"), String.fromCharCode(parseInt(i, 10)));
        }
    }
    ...
    string = string.replace(new RegExp(this.entity_table[38], "g"), String.fromCharCode(38));
    return string;
  }

  getLigaUrtea(){
    if (this.liga == this.ligak[0].value){
      return this.ligak[0].label + ' ' + this.denboraldia;
    }else{
      return this.ligak[1].label + ' ' + this.denboraldia;
    }
  }
